...
CheckSELinux() {
	...
	if [ "$STATUS" != 'disabled' ]; then 
		...
		info "  curl -s https://raw.githubusercontent.com/devops2k18/DevOpsDecember/master/vminit.sh | sudo bash"
		exit 1
	fi
}
...
DownloadJava() {
	...
	case $1 in
		8) 
			curl -s https://github.com/devops2k18/scripts/blob/master/java-params >/tmp/java-params
